...
public class CourseController {
    ...
    public ResponseEntity<CourseResponse> createCourse(
        ...
        String rolesHeader = servletRequest.getHeader("X-User-Roles");
        String emailHeader = servletRequest.getHeader("X-User-Email");

        log.info("🔐 Authorization header: {}",
                authHeader != null ? authHeader.substring(0, Math.min(40, authHeader.length())) + "..." : "❌ missing");

        log.info("👤 X-User-Email: {}", emailHeader != null ? emailHeader : "❌ missing");
        log.info("🧩 X-User-Roles: {}", rolesHeader != null ? rolesHeader : "❌ missing");

        var command = request.toCourseCommand();
        var courseResponse = CourseResponseMapper.mapToCourseResponse(createCoursePort.createCourse(command));
